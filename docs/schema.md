...
仮想要件の「ナレッジグラフのデータ構造(ノード・エッジ設計)の検討」に
...
- `ReportEvent`は`HAS_SYMPTOM`で**複数の`Symptom`**に繋がる場合がある。ある報告書が
  「前回(R005)は取付ボルトの緩み」のように**過去の報告書IDを明示的に挙げて症状を
  再解釈・補足している**場合、参照先の`ReportEvent`にも追加のカテゴリを紐づける
  (`build_knowledge_graph.py`の`backreferences`抽出・`CYPHER_BACKREFERENCE_UPDATE`)。
  この追加分の`HAS_SYMPTOM`関係には`inferred_from`プロパティを付与し、
  その報告書自身の記述による分類か、後続レポートの言及による推定分類かを
  区別できるようにしている。設備の全履歴を毎回読み直すのではなく、
  新しいレポート自身の文中にある後方参照だけを見て軽量に差分更新する設計のため、
  履歴が何万件に増えてもレポート1件あたりの処理コストは変わらない。
