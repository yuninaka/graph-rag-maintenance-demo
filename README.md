...
Symptomノードの名寄せ(カテゴリ辞書によるMERGE)と、後方参照検出による差分更新
(「前回(R005)は...」のように過去レポートを明示的に再解釈する記述を検出し、
参照先レポートへ軽量に追加カテゴリを反映する、履歴件数に依存せずスケールする設計)
を実装。途中、以前の実装に潜んでいた「原因のクロス混入」バグ(Q5・Q9で本来無関係な
設備の原因まで混入していた)も発見・修正した。残るQ1(0.25)・Q9(0.50)は、キーワード
網羅率という評価方法上`report_id`を要求する期待値にCypherの回答が応えられていない
という、実装ではなく評価設計側の課題。詳細は `docs/troubleshooting_log.md` を参照。
