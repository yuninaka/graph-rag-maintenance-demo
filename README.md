...
Symptomノードの名寄せ(カテゴリ辞書によるMERGE)を実装した結果、Q1(0.00)は0.25に改善し、
以前の実装に潜んでいた「原因のクロス混入」バグ(Q5・Q9で本来無関係な設備の原因まで
混入していた)も発見・修正できた。ただしキーワード網羅率という評価方法上、
`report_id`を要求する期待値に対しCypherの回答がreport_idを返さない設計であることや、
「1レポート=1症状カテゴリ」という単純化の限界により、数値としては直前(0.89)より下がっている。
中身としてはより正確な結果になっている点も含め、詳細は `docs/troubleshooting_log.md` を参照。
